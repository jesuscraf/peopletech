...
#ifndef __SLI_FIXED__HPP__
#define __SLI_FIXED__HPP__
...
class _ClSlices_Fixed: public _ClVirtual_Value_W
{
public:
	...
	m4return_t Serialize (ClGenericIODriver& IOD, m4pcchar_t ai_poPtrSlice, m4char_t ai_iCppType, void **ao_valueadd = 0);
...
private:
	...
	m4return_t SingleInsert(const StKernelContext &ai_context, m4uint32_t *SliceHeader, m4uint32_t ai_index, void **ao_valueadd = 0);

	m4uint32_t FreeSpace(const StKernelContext &ai_context, void **ao_valueadd = 0);

	m4return_t SetNoNull (const StKernelContext &ai_context, m4uint32_t ai_index, void **ao_valueadd = 0);

    m4return_t ValidateIndex(m4uint32_t * aio_address, m4uint32_t ai_index, const StKernelContext &ai_context, m4uint32_t ai_ErrorCode, void **ao_valueadd = 0)
    {
        if (aio_address)
        {
	        m4uint32_t n = aio_address[0];
            if (ai_index >= n)
            {
