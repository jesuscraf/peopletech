...
#ifndef __MUXWINOBL_HPP__
#define __MUXWINOBL_HPP__
...
///// funciones por hacer para el M4Listen
int M4CreateProcess ( char *ai_pcExe, t_PROCESS *ao_phPID );
void is_alive ( int a_isig);
int init_main();
int ExitProcess (int ai_iStatus);
int TerminateProcess( t_PROCESO ai_PID, int ai_iExitcode);
int GetExitCodeProcess ( t_PROCESO ai_PId, unsigned long*ao_iExitcode);
...
////   GLOBAL WINAPI AllocResource(HINSTANCE, HRSRC, DWORD);
...
////   INT    WINAPI EnumClipboardFormats(UINT);
