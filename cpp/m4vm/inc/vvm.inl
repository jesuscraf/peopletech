...
inline m4return_t ClVVM::RuntimePushString(const m4char_t *ai_pszTemp, m4bool_t ai_bIsUser){
	m4VariantType vTemp;
	vTemp.IsUser=ai_bIsUser;
	vTemp.Type=M4CL_CPP_TYPE_STRING_POOL;
	m_oDynamicPool->AddString(ai_pszTemp,vTemp.Data.CharData);
	return (m_oRunTimeStack->Push(vTemp));
}
...
inline m4return_t ClVVM::RuntimePopNumber(m4VariantType &ai_vTemp){
	...
	if (ai_vTemp.Type!=M4CL_CPP_TYPE_NUMBER && ai_vTemp.Type!=M4CL_CPP_TYPE_DATE){
		...
		DUMP_CHLOG_DEBUGF ( M4_CH_VM_RTS_POP, "Error performing pop of a number from RTS.");
	}
	m_oRunTimeStack->Pop();	
	return ret;	
}
...
inline m4return_t ClVVM::RuntimePopDate(m4VariantType &ai_vTemp){
	...
	if (ai_vTemp.Type!=M4CL_CPP_TYPE_DATE){
		ret=M4_ERROR;
		DUMP_CHLOG_DEBUGF ( M4_CH_VM_RTS_POP,  "Error performing pop of a date from RTS.");
	}
	m_oRunTimeStack->Pop();	
	return ret;	
}
...
inline m4return_t ClVVM::RuntimePopString(m4VariantType &ai_vTemp){
	...
	if (ai_vTemp.Type!=M4CL_CPP_TYPE_STRING_POOL){
		ret=M4_ERROR;
		DUMP_CHLOG_DEBUGF ( M4_CH_VM_RTS_POP, "Error performing pop of a string from RTS.");
	}
	m_oRunTimeStack->Pop();	
	return ret;	
}
...
inline m4return_t ClVVM::RuntimePopAndDelete(void){
	...
	if (ret!=M4_SUCCESS){
		DUMP_CHLOG_DEBUGF ( M4_CH_VM_RTS_POP,  "Error performing pop from RTS.");
		return ret;
	}
	m_oRunTimeStack->Pop(); 
...
inline m4bool_t ClVVM::IsNull (const m4VariantType &ai_var){
	return ( M4_BOOL(ai_var.Type == M4CL_CPP_TYPE_NULL) );
}
...
//FreePoolString
//
inline m4return_t ClVVM::FreePoolStrings (m4VariantType * ai_pavVariants , m4int32_t ai_iNumVariants )
{
	
	if ( ai_pavVariants == NULL || ai_iNumVariants ==0 )	return M4_SUCCESS;
	
	for (m4int32_t i = 0; i < ai_iNumVariants ; i++)
		
		if ( ai_pavVariants[i].Type == M4CL_CPP_TYPE_STRING_POOL )
			
			m_oDynamicPool ->DeleteString (ai_pavVariants[i].Data.CharData ) ;
