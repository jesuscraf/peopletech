...
// -- 
// -- "Meta4 Application Server Site Dispatcher (ServiceId)"
// ----------------------------------------------------------------------------
...
static m4return_t	_GetCurrentServiceName( m4pchar_t& ao_pcServiceName )
{
	...
	DOMNode			*poNode = NULL ;
	m4pchar_t		pcSiteId = NULL ;
	XercesDOMParser	oParser ;


	pccConfigFile = ClAdminAgent::GetDspConfigPath() ;

    poNode = GetConfigNode( pccConfigFile, oParser ) ;

    if( poNode != NULL )
    {
        pcSiteId = SnmpDOMUtils::getAttVal( poNode, M4_XERCES_LITERAL("site_id") ) ;
    }

	iRet = _BuildServiceName( pcSiteId, ao_pcServiceName ) ;
...
void PrintOptions()
{
	...
	cout << "	No parameters (to start the current Dispatcher service)" << endl ;
	cout << "	-install (to install the Dispatcher as a Service)" << endl;
	cout << "	-install SiteId (to install the Dispatcher as a service specifying the Server Site Id)" << endl ;
	cout << "	-erase (to stop and uninstall or uninstall the Dispatcher as a Service)" << endl;
	cout << "	-erase SiteId (to stop and uninstall or uninstall the Dispatcher as a service specifying the Server Site Id)" << endl ;
	cout << "	-run <Configuration_File_Path> (to run the Dispatcher as an executable - no Service)" << endl;
}

void Daemon_main(m4pchar_t ai_pszConfigPath = 0)
{
 ...
 //   DebugBreak();
	...
	poDpchMain->StartDispatcher(ai_pszConfigPath);


	M4_SRV_GENERAL_BLOCK_N( BlkEnd, "M4 Application Server Site Dispatcher Stopping" ) ;

	ClDpchMainSingleton::FreeInstance();
...
// del servcicio mediante la llamada a StartServiceCtrlDispatcher
		...
		case 2:
			if((*argv[1] == '-') || (*argv[1] == '/')) 
			{
				...
				{
					M4_SRV_ERROR_N(0, "Error: invalid option");
					PrintOptions();
				}
			}
			else
		...
		case 3:
			...
			{			
				if ( _stricmp( "run", argv[1]+1 ) == 0 )
				{
					...
					{
						M4_SRV_ERROR_N(0, "Error: no configuration file specified");
						PrintOptions();
					}
				}
	...
	while(estado!=0)
	{
		...
		M4_SRV_GENERAL_N("Starting the service...\n\n");
		...
		else if(pid==0)
		{
				...
				M4_SRV_GENERAL("Waiting %0:d seconds for closing the ports",tcp_close_wait);
				...
				wait(&estado);		
				M4_SRV_GENERAL( "The Dispatcher has finished in status %0:s" , estado );
	...
	M4_SRV_GENERAL_N( "The while loop only can be broken if a Stop_Daemon() were runned" ) ;
...
void SvcDebug(const char* ai_message, long ai_code) 
{
    SvcError(ai_message, ai_code);
}


int getInstallDir( char* ai_pcServiceName, char *ao_installdir )
{
    SvcDebug(">>>getInstallDir", 0);
    int retCode = -1;
    ...
    if (schService == NULL) 
    {
        SvcError("Error in OpenService. ErrorCode: %ld", GetLastError());
        return retCode;
    }
    ...
    if (lpqscBuf == NULL) 
    {
        SvcError("Error in LocalAlloc. ErrorCode: %ld", GetLastError());
        return retCode;
    }
    ...
    if (! QueryServiceConfig( 
        ...
        &dwBytesNeeded) ) 
    {
        SvcError("Error in QueryServiceConfig. ErrorCode: %ld", GetLastError());
        return retCode;
    }
    ...
    if (sshStatusHandle)
	{
		...
		{					   // cuando se pasa un estado PENDING
            ...
            if (!getInstallDir( *lpszArgv, installdir ))
            {
                ...
                if (_chdir(installdir))
                {
                    SvcError("Error in _chdir. ErrorCode: %ld", GetLastError());
                    SvcError(installdir, -1);
                }
            }

			// Y ahora la rutina principal.
            Daemon_main ();
			//*************************************************
			//*************************************************
		}
        ...
        {
            SvcError("Error in ServiceMain.ReportStatusToSCMgr. ErrorCode: %ld", GetLastError());
        }

	}
    else
    {
        SvcError("Error in ServiceMain.RegisterServiceCtrlHandler. ErrorCode: %ld", GetLastError());
    }
...
//////////////////////////////////////////////////////////////
 //   DebugBreak();
    ...
    //
    SvcDebug(">>>service_ctrl:", 0);
    switch(dwCtrlCode)
    {
        // Parar el servicio.
        //
        case SERVICE_CONTROL_STOP:
            SvcDebug("service_ctrl.SERVICE_CONTROL_STOP:", 0);
            ...
            break;
        default:
            SvcDebug("service_ctrl.default:", 0);
            break;
...
//////////////////////////////////////////////////////////////
	...
	SvcError ("\nInstalling the Site Dispatcher as a Windows Service...", 0);
    ...
    if ( GetModuleFileName( NULL, szPath, 2048 ) == 0 )
    {
		...
		SvcError ("\nERROR when installing Site Dispatcher as a Windows Service:\n", 0, cerr);
        SvcError("Error in GetModuleFileName. ErrorCode: %ld", GetLastError(), cerr);		
		SvcError ("Site Dispatcher could not be installed as a Windows Service.", 0, cerr);       
        return 1;
    }
    ...
    if ( schSCManager )
    {
        ...
        if ( schService )
        {			
			SvcError ("\nSite Dispatcher has been installed as a Windows Service with name: ", 0);
...
//////////////////////////////////////////////////////////////
	...
	SvcError ("Removing the Site Dispatcher from the Windows Services list...", 0);
    ...
    if ( schSCManager )
    {
        ...
        if (schService)
        {
			...
			{
				SvcError ("\nSite Dispatcher has been removed from the Windows Services list.", 0);
				M4_SRV_GENERAL("%0:s eliminado.\n", ai_pccDisplayName);
				iRet = 0;
			}
            else
			{
				SvcError ("\nERROR when removing Site Dispatcher Windows Service:\n", 0, cerr);
				SvcError("Error in DeleteService. ErrorCode: %ld", GetLastError(), cerr);		
				SvcError ("Site Dispatcher could not be removed.", 0, cerr);   
...
m4return_t Stop_Daemon()
{
	...
	M4_SRV_GENERAL_N( "Iniciando shutdown del M4 Application Server Site Dispatcher" );
	...
	(void) ClDpchMainSingleton::FreeInstance();
