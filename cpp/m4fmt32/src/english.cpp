...
m4int16_t ENG_NumberToText(ClCadena *a_pCad, ClCadena *a_pNum, m4bool_t bGender, m4bool_t bIsDate)
{
	...
	if(!a_pNum->EsNula()){   
		...
		for(;;){   
			if(nmill==1){ 
				m4uint32_t numero=ENG_CenDecUni(a_pCad,millones,&bHay);
				if(numero){
					a_pCad->EscribeCadena(s_xllones[0]);
				};
				anterior=numero*1000+ENG_CenDecUni(a_pCad,millones+3,&bHay);
			};
			nmill--;
			if(nmill){ 
				m4uint32_t numero=ENG_CenDecUni(a_pCad,millones,&bHay);
...
m4uint32_t ENG_CenDecUni(ClCadena *a_pCad, m4char_t* a_cendecuni, m4bool_t* pbHay, m4uint32_t tamgrup) 
{
...
const m4char_t* ENG_GetMonth(m4uint16_t ai_iMes)
{		
	return s_meses[ai_iMes-1];
}

const m4char_t* ENG_GetDay(m4uint16_t ai_iDia)
{
...
m4int16_t ENG_YearToText(ClCadena *a_pCad, ClCadena *a_pNum, m4bool_t bGender) 
{
	...
	if (cifras<4) {
		if (cifras==3){
			...
			bHay=M4_FALSE;
			m4uint32_t numero=ENG_CenDecUni(a_pCad, s, &bHay);
			a_pCad->EscribeChar(' ');
			a_pNum->LeeCadena(s+1,2);
			bHay=M4_FALSE;
			numero=ENG_CenDecUni(a_pCad, s, &bHay);
		}
	}
	else
	{
		...
		bHay=M4_FALSE;
		if(!strcmp(s,"020")){
			ENG_NumberToText(a_pCad, a_pNum, bGender, M4_TRUE); 
			return 1;
		};
		m4uint32_t numero=ENG_CenDecUni(a_pCad, s, &bHay);				
		a_pCad->EscribeChar(' ');
...
{ 
	...
	if(!a_pNum->EsNula()){   
		...
		for(;;){   
			...
			a_pCad->EscribeChar(' ');
			anterior=ENG_CenDecUni(a_pCad, s, &bHay, tamgrup);  //Grupo debe escribir primero los ceros
			//otro grupo
			grupos--;
