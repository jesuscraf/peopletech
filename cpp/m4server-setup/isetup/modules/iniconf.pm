...
# @(#) FileDescription: IniConf provides a way to have readable configuration files
...
#
#--------------------------------------------------------------------------------
# Meta 4 modifications:
# 	- include traces with m4trace module.
#	- when the user ask for an inexistent section, the module crash. FIXED.
#	- delete initial and final whitespaces in value returned by funcion val()
#	- if a key value has the character '=' the parser fails. FIXED
#	- allow '<' character as a comment character
...
sub val {
  ...
  else 
  {
	...
	# delete whitespaces.
	$g_oTrace->Print("Value read is: #$retVal#", $g_errorBase+1);
	
	# subst any white_spaces secuence by only one whitespace.
	$retVal =~ s/\s\s+/ /g;
	...
	$g_oTrace->Print("New Value read is: #$retVal#", $g_errorBase+1);
	return $retVal;
}
...
sub setval {
  ...
  {
  	$g_oTrace->Print("Can't set INI section: $sect, $parm", $g_errorBase+1);
    return undef;
  }
}
...
sub ReadConfig {
  ...
  while (<CF>) {
    ...
    #--- old code. elsif (($parm, $val) = /\s*(\S+)\s*=\s*(.*)/) {	# new parameter
	#--- if the value has the character '=', the above sentence fails.
    elsif (($parm, $val) = /\s*([^\s=]+)\s*=\s*(.*)/) {	# new parameter
		$g_oTrace->Print("(parm, val) = $parm:--:$val", $g_errorBase+3);
...
sub Parameters 
{
	...
	if (!defined($sect))
	{
		$g_oTrace->Print("Section $sect doesn't exist in file $file.", $g_errorBase+1);
		return @parameters;
	}
  
	if (defined($self->{parms}{$sect}))
	{
		@parameters = @{$self->{parms}{$sect}};
	}
	else
	{
		$g_oTrace->Print("Section $sect doesn't exist in file $file.", $g_errorBase+1);
