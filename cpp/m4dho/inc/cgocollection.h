...
// Construction
    ...
    {	
		...
		m_nOrderType = 0;
		m_ppHashTable = (CMinNode**)malloc(HASH_SIZE*sizeof(CMinNode*));
		memset(m_ppHashTable, 0, HASH_SIZE*sizeof(CMinNode*));
    };
...
private:
	...
	CMinNode * AddTail(TYPE *pIType, BSTR bstrKey);
	...
	void InsertNodeInHash(CMinNode* pNewNode);
};
...
template<class TYPE, class BASE>
typename CGoCollection<TYPE,BASE>::CMinNode* CGoCollection<TYPE,BASE>::GetNode(long lIndex)
{ 
    CMinNode* pNode=NULL;
    long lCounter=1;
...
template<class TYPE, class BASE>
typename CGoCollection<TYPE,BASE>::CMinNode* CGoCollection<TYPE,BASE>::GetNode(BSTR bstrKey)
{ 
	USES_CONVERSION;
	...
	while (p && wcscmp(p->bstrKey, bstrKey))
	{
		p = p->pNextHash;
	}
...
template<class TYPE, class BASE>
typename CGoCollection<TYPE, BASE>::CMinNode* CGoCollection<TYPE,BASE>::AddTail(TYPE *pIType, BSTR bstrKey)
{
	USES_CONVERSION;
...
template<class TYPE, class BASE>
typename CGoCollection<TYPE,BASE>::CMinNode* CGoCollection<TYPE,BASE>::NewMinNode(
	CMinNode* pPrev,
	CMinNode* pNext)
{
    CGoCollection<TYPE,BASE>::CMinNode* pNode = new CGoCollection<TYPE,BASE>::CMinNode;
	pNode->pPrev = pPrev;
	pNode->pNext = pNext;
...
template<class TYPE, class BASE>
void CGoCollection<TYPE,BASE>::FreeMinNode(CMinNode* pNode)
{
	...
	if (pNode->bstrKey)
	{
		...
		int nHash = HashKey(cTemp) % HASH_SIZE;
		CMinNode * pBefore = NULL;
		CMinNode * p = m_ppHashTable[nHash];
		while (p && wcscmp(p->bstrKey, pNode->bstrKey))
		{
			pBefore = p;
			p = p->pNextHash;
		}
...
template<class TYPE, class BASE>
void CGoCollection<TYPE,BASE>::OrderQuick(CMinNode* first, CMinNode* last)
{
	
	CGoCollection<TYPE,BASE>::CMinNode* Pivot;
	CGoCollection<TYPE,BASE>::CMinNode* aNodelo = first;
	CGoCollection<TYPE,BASE>::CMinNode* aNodehi = last;
...
template<class TYPE, class BASE>
long CGoCollection<TYPE,BASE>::CompareNodes(CMinNode* pBefore, CMinNode* pAfter)
{
	...
	if (m_nOrderType == 2)
	{
		return wcscmp(pBefore->bstrOrder, pAfter->bstrOrder);
	}
	else
	{
...
template<class TYPE, class BASE>
void CGoCollection<TYPE,BASE>::SwapNodes(CMinNode* pBefore, CMinNode* pAfter)
{
	CMinNode Buffer;
...
template<class TYPE, class BASE>
void CGoCollection<TYPE,BASE>::InsertNodeInHash(CMinNode* pNewNode)
{
	USES_CONVERSION;
