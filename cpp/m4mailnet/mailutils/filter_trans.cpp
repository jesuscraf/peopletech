...
{
  struct _trans_stream *ts = (struct _trans_stream *)filter->data;
  if (ts->s_buf)
    free (ts->s_buf);
  free (ts);
}
...
b64_input (char c)
{
  /*error C2117: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/' : array bounds overflow */
  const char table[64+1] =
...
base64_init (filter_t filter)
{
  ...
  ts = (struct _trans_stream *)calloc (sizeof (*ts), 1);
  if (ts == NULL)
    return ENOMEM;

  ts->min_size = 4;
  ts->s_buf = (char*)calloc (4, 1);
  if (ts->s_buf == NULL)
    {
      free (ts);
      return ENOMEM;
...
/* error C2117: '0123456789ABCDEF' : array bounds overflow */
static const char _hexdigits[16+1] = "0123456789ABCDEF";
