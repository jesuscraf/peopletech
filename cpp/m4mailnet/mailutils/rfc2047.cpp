...
/* 
 * Meta4.
 * Minor changes to compile in C++ mode. 
 */
...
rfc2047_decode (const char *tocode, const char *input, char **ptostr)
{
  ...
  free (tmpcopy);
  *ptostr = (char*)realloc (buffer, bufpos);
  return status;
}
...
qp_count (rfc2047_encoder * enc)
{
  ...
  enc->charblock = enc->charcount ((const char *)enc->src);
...
static const char _hexdigit[16+1] = "0123456789ABCDEF";
  ...
  for (is_compose = 0 ; is_compose <= 1 ; is_compose ++) 
    {
      ...
      enc.src  = (const unsigned char *)text;
      ...
      while (enc.todo) 
	{
	  ...
	  switch (state) 
	      ...
	      if (isspace (* enc.src)) 
		{
	      ...
	      if (is_next_quoted ((const char *)enc.src)) 
		{
		  init_quoted (& enc);
		  state = ST_QUOTED;
		}
	  ...
	  state == ST_QUOTED_SPACE)
	{
      ...
      if (enc.dst == NULL) 
	{
	  enc.dst = (char*)malloc (enc.done + 1);
