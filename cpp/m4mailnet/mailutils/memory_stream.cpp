...
{
  struct _memory_stream *mfs = (struct _memory_stream *)stream_get_owner (stream);
  if (mfs && mfs->ptr != NULL)
    free (mfs->ptr);
  if(mfs->filename)
    free (mfs->filename);
	       ...
	       off_t offset, size_t *nbytes)
{
  ...
  if (mfs->capacity < ((size_t)offset + isize))
	...
	(((offset + isize)/MU_STREAM_MEMORY_BLOCKSIZE) + 1);
      char *tmp =  (char*)realloc (mfs->ptr, newsize);
      if (tmp == NULL)
	return ENOMEM;
      mfs->ptr = tmp;
...
_memory_truncate (stream_t stream, off_t len)
{
  ...
  if (len == 0)
    ...
    {
      char *tmp = (char*)realloc (mfs->ptr, len);
      if (tmp == NULL)
	return ENOMEM;
      mfs->ptr = tmp;
    }
  ...
  if (mfs->filename)
    {
      ...
      // UNICODE FILE
      int iLength = -1 ;
      char* pcFileName = M4CppToANSI( mfs->filename, iLength ) ;
      int ret = stat (pcFileName, &statbuf) ;
      delete [] pcFileName ;

      if (ret == 0)
        {
          mfs->ptr = (char*)calloc (statbuf.st_size, 1);
          if (mfs->ptr)
            {
              FILE *fp;
              ...
              fp = M4Fopen (mfs->filename, M4UniRead, eEncoding);
              if (fp)
                {
...
memory_stream_create (stream_t * stream, const char *filename, int flags)
{
  ...
  mfs = (struct _memory_stream *)calloc (1, sizeof (*mfs));
